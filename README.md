...
- The L-function identity `L(Θ_𝓘, s) = ζ_K(s)·ζ_K(s-1)` **exactly** for
  `K = ℚ(√5)` (no local correction; `C_2 = 1`), verified by complete
  short-vector enumeration in the maximal icosian order. The classical
  Riemann zeta function ζ(s) appears as one Dedekind factor.
...
| 11 | `sim_representation_numbers.py` | Lipschitz-suborder proxy, odd π (8 units) |
| 12 | `sim_multiplicativity.py`       | normalized r multiplicative on coprime pairs |
| 13 | `sim_theta_maximal_order.py`    | **maximal order**: r(π)=120(1+N) every prime; `L(Θ_𝓘,s)=ζ_K(s)·ζ_K(s-1)` **exactly** (C_2=1) |
...
- [`icosian-closure-object`](https://github.com/vfd-org/icosian-closure-object)
  (v0.1.0-review) builds directly on this bundle's exact identity
  `L(Θ_𝓘,s) = ζ_K(s) ζ_K(s−1)` (C₂ = 1). It is the **reach-and-limit + witness**
